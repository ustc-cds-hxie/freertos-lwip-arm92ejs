...
int cmd_iperfapp(int argc, char **argv)
{
    ...
    if (param.mode == IPERF_MODE_STOP)
    {
        ...
        if (host) param.host = "10.0.2.15";
        //mode = IPERF_MODE_SERVER;
        if (mode == IPERF_MODE_CLIENT)
        {
          sys_thread_new(TCP_SERVER_THREAD_NAME, iperf_client, NULL, 4096, 4); 
        }                        
        else if (mode == IPERF_MODE_SERVER)
            sys_thread_new(TCP_SERVER_THREAD_NAME, iperf_server, NULL, 4096, 4);
    }
    else
    {
